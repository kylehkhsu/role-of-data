...
def main(args):
    ...
    train_set_size_ghost = int(0.5 * train_set_size_all)
    train_set_size_not_ghost = train_set_size_all - train_set_size_ghost
    train_set_size_alpha = int(config.alpha * train_set_size_not_ghost)
    train_set_size_not_alpha = train_set_size_not_ghost - train_set_size_alpha
    train_set_size_fill = train_set_size_ghost - train_set_size_alpha
    train_set_size_unused = train_set_size_ghost - train_set_size_fill

    train_set_ghost, train_set_not_ghost = data.random_split(
        train_set, [train_set_size_ghost, train_set_size_not_ghost]
    )

    train_set_alpha, train_set_not_alpha = data.random_split(
        train_set_not_ghost, [train_set_size_alpha, train_set_size_not_alpha]
    )

    train_set_fill, train_set_unused = data.random_split(
        train_set_ghost, [train_set_size_fill, train_set_size_unused]
    )

    batch_size_eval = 10000

    # S
    train_loader_not_ghost = data.DataLoader(
        dataset=train_set_not_ghost,
    ...
    train_loader_eval_not_ghost = data.DataLoader(
        dataset=train_set_not_ghost,
    ...
    # S_alpha \cup S_fill
    train_set_size_alpha_and_fill = train_set_size_alpha + train_set_size_fill
    train_set_alpha_and_fill = data.ConcatDataset([train_set_alpha, train_set_fill])
    assert train_set_size_alpha_and_fill == train_set_size_not_ghost
    assert train_set_size_fill == train_set_size_not_alpha
    train_loader_alpha_and_fill = data.DataLoader(
        dataset=train_set_alpha_and_fill,
        batch_size=config.batch_size,
        shuffle=True,
        num_workers=2
    )
    train_loader_eval_alpha_and_fill = data.DataLoader(
        dataset=train_set_alpha_and_fill,
        batch_size=batch_size_eval,
        num_workers=2
    )
    ...
    pp.pprint({
        'train_set_size_all': train_set_size_all,
        'train_set_size_not_ghost': train_set_size_not_ghost,
        'train_set_size_ghost': train_set_size_ghost,
        'train_set_size_not_alpha': train_set_size_not_alpha,
        'train_set_size_alpha': train_set_size_alpha,
        'train_set_size_fill': train_set_size_fill,
        'train_set_size_alpha_and_fill': train_set_size_alpha_and_fill,
    ...
    for i_epoch in tqdm(range(1, config.n_epoch_posterior_mean)):
        log = train_classifier_epoch(
            classifier=classifier_posterior_mean,
            optimizer=optimizer_posterior_mean,
            train_loader=train_loader_not_ghost,
            train_set_size=train_set_size_not_ghost,
            train_loader_eval=train_loader_eval_not_ghost,
            test_loader=test_loader,
            config=config
        )
        log.update({
    ...
    for i_epoch in tqdm(range(1, config.n_epoch_prior_mean)):
        log = train_classifier_epoch(
            ...
            train_loader=train_loader_alpha_and_fill,
            train_set_size=train_set_size_alpha_and_fill,
            train_loader_eval=train_loader_eval_alpha_and_fill,
            test_loader=test_loader,
            config=config
        )
